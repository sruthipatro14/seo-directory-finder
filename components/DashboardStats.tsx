...
function StatCard({ label, value, sublabel, icon, accent }: StatCardProps) {
  return ( // Changed rounded-2xl to rounded-xl, shadow-sm to shadow-lg
    <div className="rounded-xl border border-slate-700 bg-slate-800 p-5 shadow-lg flex flex-col gap-4">
      {/* Top row — label + icon */}
      ...
      {/* Value */}
      <div>
        <p className="text-3xl font-bold text-slate-50 leading-none">
          {value}
        </p>
        <p className="text-xs text-slate-300 mt-1.5">{sublabel}</p>
      </div>
